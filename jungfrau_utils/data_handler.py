...
class JFDataHandler:
    ...
    def process(
        ...
        *,
        ...
        if self.detector_name.startswith("JF18"):
            reshape_stripsel = _reshape_stripsel_jf18
        else:
        ...
        for i, m in enumerate(self.module_map):
            ...
            if self.detector_geometry.is_stripsel and geometry:
                ...
                _adc_to_energy(mod_tmp, mod, mod_g, mod_p, mod_mask, self.factor, ry, rx)
                reshape_stripsel(mod_out, mod_tmp)
            else:
                mod_out = np.rot90(mod_out, k=-rot90, axes=(1, 2))
                _adc_to_energy(mod_out, mod, mod_g, mod_p, mod_mask, self.factor, ry, rx)
                if double_pixels == "interp":
                    _inplace_interp_dp(mod_out)
    ...
    def get_pixel_mask(
        ...
        for i, m in enumerate(self.module_map):
            ...
            if self.detector_geometry.is_stripsel and geometry:
                ...
                _adc_to_energy(mod_res, mod, None, None, None, None, ry, rx)
                if double_pixels == "interp":
...
def _inplace_mask_dp(res: NDArray) -> None:
